...
Feature: API Aerolineas
  ...
  @CP03
  Scenario Outline: Eliminar una aerolinea exitosamente
    Given el actor establece el endpoint para eliminar la aerolinea
    When el envia una solicitud HTTP DELETE con el "<_id>"
    Then el codigo de respuesta deberia ser 200
  ...
  @CP04
  Scenario Outline: Obtener la aerolínea por id exitosamente
    Given el actor establece el endpoint para obtener la aerolinea por id
    When el envia una solicitud HTTP GET con el "<_id>"
    Then el codigo de respuesta deberia ser 200
